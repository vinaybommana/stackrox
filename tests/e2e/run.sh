...
run_proxy_tests() {
    ...
    for p in "${proxies[@]}"; do
        ...
        roxctl central debug log "${extra_args[@]}" --plaintext="$plaintext" -e "${endpoint_tgt}" -p "$ROX_PASSWORD" >/dev/null || \
            failures+=("$p")

        if (( direct )); then
            roxctl central debug log "${extra_args[@]}" --plaintext="$plaintext" --force-http1 -e "${endpoint_tgt}" -p "$ROX_PASSWORD" &>/dev/null && \
        ...
        if [[ "$endpoint_tgt" = *://* ]]; then
            ...
            roxctl central debug log "${extra_args[@]}" -e "${endpoint_tgt}" -p "$ROX_PASSWORD"  >/dev/null || \
            failures+=("${p},tls-autosense")

            # Incompatible plaintext configuration should fail
            roxctl central debug log "${extra_args[@]}" --plaintext="$plaintext_neg" -e "${endpoint_tgt}" -p "$ROX_PASSWORD" &>/dev/null && \
            failures+=("${p},incompatible-tls")
        fi

        done
        roxctl sensor generate k8s "${extra_args[@]}" --plaintext="$plaintext" -e "${server_name}:${port}" -p "$ROX_PASSWORD" --name remote --continue-if-exists || \
