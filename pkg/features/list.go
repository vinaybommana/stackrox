...
var (
	...
	ComplianceOperatorCheckResults = registerUnchangeableFeature("Enable fetching of compliance operator results", "ROX_COMPLIANCE_OPERATOR_INTEGRATION", true)

	// SystemHealthPatternFly enables the Pattern Fly version of System Health page. (used in the front-end app only)
	SystemHealthPatternFly = registerUnchangeableFeature("Enable Pattern Fly version of System Health page", "ROX_SYSTEM_HEALTH_PF", false)
	...
	NetworkPolicySystemPolicy = registerUnchangeableFeature("Enable NetworkPolicy-related system policy fields", "ROX_NETPOL_FIELDS", true)
	...
	RoxctlNetpolGenerate = registerUnchangeableFeature("Enable 'roxctl generate netpol' command", "ROX_ROXCTL_NETPOL_GENERATE", true)

	// NetworkGraphPatternFly enables the PatternFly version of NetworkGraph. (used in the front-end app only)
	NetworkGraphPatternFly = registerUnchangeableFeature("Enable PatternFly version of NetworkGraph", "ROX_NETWORK_GRAPH_PATTERNFLY", true)

	// ClairV4Scanner enables Clair v4 as an Image Integration option
	ClairV4Scanner = registerUnchangeableFeature("Enable Clair v4 as an Image Integration option", "ROX_CLAIR_V4_SCANNING", true)
	...
	VulnMgmtWorkloadCVEs = registerUnchangeableFeature("Vuln Mgmt Workload CVEs", "ROX_VULN_MGMT_WORKLOAD_CVES", false)
	...
	VulnMgmtReportingEnhancements = registerUnchangeableFeature("Vuln Mgmt Reporting Enhancements", "ROX_VULN_MGMT_REPORTING_ENHANCEMENTS", false)

	// StoreEventHashes stores the hashes of successfully processed objects we receive from Sensor into the database
	StoreEventHashes = registerUnchangeableFeature("Store Event Hashes", "ROX_STORE_EVENT_HASHES", false)

	// PostgresDatastoreEnabled toggles whether to use Postgres for the datastore or not.
	PostgresDatastoreEnabled = registerUnchangeableFeature("Enable PostgresSQL Datastore", "ROX_POSTGRES_DATASTORE", true)
	...
	SecuredClusterNotifiers = registerFeature("Enable Secured Cluster Notifiers", "ROX_SECURED_CLUSTER_NOTIFICATIONS", false)
)
