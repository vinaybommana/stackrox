...
func handleAllEvents(sif, resyncedSif informers.SharedInformerFactory, osf externalversions.SharedInformerFactory, output chan<- *central.SensorEvent, stopSignal *concurrency.Signal, config config.Handler) {
	...
	podInformer := resyncedSif.Core().V1().Pods()
	...
	roleInformer := resyncedSif.Rbac().V1().Roles().Informer()
	clusterRoleInformer := resyncedSif.Rbac().V1().ClusterRoles().Informer()
	roleBindingInformer := resyncedSif.Rbac().V1().RoleBindings().Informer()
	clusterRoleBindingInformer := resyncedSif.Rbac().V1().ClusterRoleBindings().Informer()
	...
	resyncedSif.Start(stopSignal.Done())
	...
	handle(resyncedSif.Batch().V1().Jobs().Informer(), dispatchers.ForDeployments(kubernetes.Job), output, &treatCreatesAsUpdates, preTopLevelDeploymentWaitGroup, stopSignal, &eventLock)
	handle(resyncedSif.Apps().V1().ReplicaSets().Informer(), dispatchers.ForDeployments(kubernetes.ReplicaSet), output, &treatCreatesAsUpdates, preTopLevelDeploymentWaitGroup, stopSignal, &eventLock)
	handle(resyncedSif.Core().V1().ReplicationControllers().Informer(), dispatchers.ForDeployments(kubernetes.ReplicationController), output, &treatCreatesAsUpdates, preTopLevelDeploymentWaitGroup, stopSignal, &eventLock)
	...
	// Deployment types.
	handle(resyncedSif.Apps().V1().DaemonSets().Informer(), dispatchers.ForDeployments(kubernetes.DaemonSet), output, &treatCreatesAsUpdates, wg, stopSignal, &eventLock)
	handle(resyncedSif.Apps().V1().Deployments().Informer(), dispatchers.ForDeployments(kubernetes.Deployment), output, &treatCreatesAsUpdates, wg, stopSignal, &eventLock)
	handle(resyncedSif.Apps().V1().StatefulSets().Informer(), dispatchers.ForDeployments(kubernetes.StatefulSet), output, &treatCreatesAsUpdates, wg, stopSignal, &eventLock)
	handle(resyncedSif.Batch().V1beta1().CronJobs().Informer(), dispatchers.ForDeployments(kubernetes.CronJob), output, &treatCreatesAsUpdates, wg, stopSignal, &eventLock)
