...
func init() {
	framework.MustRegisterChecks(
		...
		runningContainerCheck("CIS_Docker_v1_2_0:5_10", memoryLimit, "has memory limits configured"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_11", cpuShares, "has CPU shares configured"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_12", readonlyFS, "uses a read-only root filesystem"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_13", specificHostInterface, "does not bind to all host interface addresses (0.0.0.0)"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_14", restartPolicy, "has an on-failure restart policy with a maximum of 5 retries"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_15", pidNamespace, "is not using the host PID namespace"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_16", ipcNamespace, "is not using the host IPC namespace"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_17", hostDevices, "has not mounted any host devices"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_18", ulimit, "does not override ulimits"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_19", mountPropagation, "does not have any mounts that use shared propagation"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_20", utsNamespace, "does not use the host UTS namespace"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_21", seccomp, "does not have seccomp set to unconfined"),

		// 5.22 and 5.23 are in file.go

		runningContainerCheck("CIS_Docker_v1_2_0:5_24", cgroup, "does not use a non-standard cgroup parent"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_25", acquiringPrivileges, "sets no-new-privileges in its security options"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_26", healthcheck, "has a correctly configured health check"),
		common.PerNodeNoteCheck("CIS_Docker_v1_2_0:5_27", "Pulling images is invasive and not always possible depending on credential management"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_28", pidCgroup, "has a PID limit set"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_29", bridgeNetwork, "is not running on the bridge network"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_30", userNamespace, "is not using the host user namespace"),
		runningContainerCheck("CIS_Docker_v1_2_0:5_31", noDockerSocket, "does not mount the docker socket"),
