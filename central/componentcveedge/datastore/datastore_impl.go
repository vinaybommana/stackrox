...
func (ds *datastoreImpl) Upsert(ctx context.Context, edge *storage.ComponentCVEEdge) error {
	if edge.GetId() == "" {
		return errors.New("cannot upsert an component/cve edge without an id")
	}
	...
	if err := ds.indexer.AddComponentCVEEdge(edge); err != nil {
		return err
	}
...
func (ds *datastoreImpl) Delete(ctx context.Context, ids ...string) error {
	...
	errorList := errorhelpers.NewErrorList("deleting component/cve edges")
	for _, id := range ids {
		if err := ds.storage.Delete(id); err != nil {
			errorList.AddError(err)
			continue
		}
		if err := ds.indexer.DeleteComponentCVEEdge(id); err != nil {
			errorList.AddError(err)
		}
	}
