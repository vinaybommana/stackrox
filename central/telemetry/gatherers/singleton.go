...
func Singleton() *CentralGatherer {
	gathererInit.Do(func() {
		gatherer = NewCentralGatherer(singleton.ManagerSingleton(), newDatabaseGatherer(newBadgerGatherer(globaldb.GetGlobalBadgerDB()), newBoltGatherer(globaldb.GetGlobalDB())), newAPIGatherer(metrics.Singleton()), gatherers.NewComponentInfoGatherer())
	})
	return gatherer
}
