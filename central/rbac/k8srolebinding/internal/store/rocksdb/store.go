...
func New(db *gorocksdb.DB) Store {
	globaldb.RegisterBucket(bucket, "K8SRoleBinding")
	return &storeImpl{
...
func (b *storeImpl) Count() (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "K8SRoleBinding")
...
func (b *storeImpl) Exists(id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "K8SRoleBinding")
...
func (b *storeImpl) GetIDs() ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "K8SRoleBindingIDs")

	return b.crud.GetKeys()
}

// Get returns the object, if it exists from the store
func (b *storeImpl) Get(id string) (*storage.K8SRoleBinding, bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Get, "K8SRoleBinding")
...
func (b *storeImpl) GetMany(ids []string) ([]*storage.K8SRoleBinding, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "K8SRoleBinding")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.K8SRoleBinding, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.K8SRoleBinding))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(obj *storage.K8SRoleBinding) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "K8SRoleBinding")

	return b.crud.Upsert(obj)
}

// UpsertMany batches objects into the DB
func (b *storeImpl) UpsertMany(objs []*storage.K8SRoleBinding) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "K8SRoleBinding")
...
func (b *storeImpl) Delete(id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "K8SRoleBinding")
...
func (b *storeImpl) DeleteMany(ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "K8SRoleBinding")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(fn func(obj *storage.K8SRoleBinding) error) error {
